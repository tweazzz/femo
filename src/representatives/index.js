...
async function loadRepresentativeProfileForHeader() {
  try {
    ...
    currentRepresentativeId = profile.profile.id;
...
async function loadCurrentOlympiad() {
  ...
  try {
    ...
    olympiad.stages.forEach((stage, idx) => {
      ...
      // ========== делаем кнопку «Подробнее» ссылкой на сайт олимпиады ==========
    const moreBtn = block.querySelector('a.btn-base.text-sm');
    if (moreBtn) {
      moreBtn.href = olympiad.website;          // URL из response
      moreBtn.target = '_blank';                // открывать в новой вкладке (опционально)
      moreBtn.rel    = 'noopener noreferrer';   // безопасность
    }
    });
...
function formatDate(isoDate) {
  ...
  function renderRankingTable(data, expanded = false) {
    ...
    visibleRows.forEach((item, index) => {
      ...
      const showRank = isTopThree ? crownSvg : item.rank

      const row = document.createElement('tr');

      // если это наша строка — сразу ставим нужный фон
      if (item.id === currentRepresentativeId) {
        row.style.backgroundColor = '#FEF1E4';
      } else {
        // иначе — ваш обычный бэкграунд
        row.className = isTopThree
          ? ''
...
async function loadRepresentativeRanking() {
  try {
    ...
    while (pageUrl) {
      ...
      if (!res.ok) {
        throw new Error(`Ошибка при получении рейтинга: ${res.status}`);
      }
      const data = await res.json();
      allResults.push(...(data.results || []));
      pageUrl = data.next ? data.next.replace('http://', 'https://') : null;
    }

    // Сохраняем в глобальную переменную
    allRankingData = allResults;

    // Рендерим (по умолчанию —collapsed)
    renderRankingTable(allRankingData, false);

    // Повесим логику «Посмотреть все / Скрыть»

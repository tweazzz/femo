...
async function loadAllTasks() {
  ...
  let url = `https://portal.femo.kz/api/olympiads/participant/dashboard/${olympiadId}/assignments/?language=${encodeURIComponent(datalang)}`;
  ...
  try {
    while (url) {
      ...
      if (Array.isArray(data)) {
        // Простой массив (как в твоём примере) — добавляем всё и выходим
        allResults.push(...data);
        url = null; // нет пагинации в простом массиве
      } else if (data && Array.isArray(data.results)) {
        // Пагинируемый ответ { results: [...], next: '...' }
        allResults.push(...data.results);
        url = data.next || null;
      } else {
        console.warn('Неожиданный формат ответа assignments:', data);
        break;
      }
    }
  ...
  async function loadAllTasksWithFilters() {
    ...
    const url = `https://portal.femo.kz/api/olympiads/participant/dashboard/${olympiadId}/assignments/?language=${datalang}/?${params.toString()}`;
    try {
      const response = await authorizedFetch(url, { headers: { Authorization: `Bearer ${token}` }});
      if (!response.ok) throw new Error(`Ошибка загрузки: ${response.status}`);
      const data = await response.json();
...
async function loadOlympiadTitleFromUrl() {
  const token = localStorage.getItem('access_token');
  if (!token) return;

  const urlParams = new URLSearchParams(window.location.search);
  const olympiadId = Number(urlParams.get('olympiadId'));
  if (!olympiadId) {
    console.error('olympiadId не найден в URL');
    return;
  }

  let url = 'https://portal.femo.kz/api/olympiads/participant/dashboard/';
  let foundOlympiad = null;

  try {
    while (url && !foundOlympiad) {
      ...
      if (Array.isArray(data.results)) {
        foundOlympiad = data.results.find(o => o.id === olympiadId);
        url = data.next;
      } else {
    ...
    if (!foundOlympiad) {
      console.warn(`Олимпиада с id=${olympiadId} не найдена`);
      return;
    }

    const titleEl = document.getElementById('name_of_olymp');
    if (titleEl) {
      titleEl.textContent = foundOlympiad.title;
    }

  } catch (err) {
    console.error('Ошибка при поиске олимпиады:', err);
  }
}
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    await loadOlympiadTitleFromUrl();
    await loadAllTasks();
